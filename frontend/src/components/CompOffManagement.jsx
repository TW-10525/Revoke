  ...
  return (
    <div className="space-y-6">
      ...
      {currentUser?.user_type === 'manager' && (
        <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-800">Grant Comp-Off to Employee (For Non-Shift Work)</h3>
            {!showCompOffForm && (
              <button
                onClick={() => setShowCompOffForm(true)}
                className="flex items-center gap-2 bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition"
          ...
          {showCompOffForm && (
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Date Employee Worked (Non-Shift Day) <span className="text-red-500">*</span>
                </label>
                <input
                  type="date"
                  value={compOffForm.comp_off_date}
                ...
                />
                <p className="text-xs text-gray-500 mt-1">
                  Select the date when the employee worked on a non-shift day (weekend/holiday).
                </p>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Reason (What work did they do?)
                </label>
                <textarea
                  value={compOffForm.reason}
                  onChange={(e) => setCompOffForm({ ...compOffForm, reason: e.target.value })}
              ...
              <div className="flex gap-2">
                <button
                  onClick={handleAddCompOff}
                  disabled={loading || !compOffForm.employee_id}
                  className="flex-1 bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition disabled:opacity-50"
                >
                  {loading ? 'Creating...' : 'Grant Comp-Off Request'}
                </button>
                <button
                  onClick={() => setShowCompOffForm(false)}
                  className="flex-1 bg-gray-300 text-gray-800 px-4 py-2 rounded-lg hover:bg-gray-400 transition"
